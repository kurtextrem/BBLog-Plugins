	...
	addList: function() {
		...
		$(this.extendedFriends).each(function(i, player) {
			var playerName = player,
				profileLink = '/bf' + (BBLog.cache('mode') === 'bf4' ? '4' : '3') + '/user/' + playerName
			$.ajax(profileLink, {
				type: 'GET',
				...
				success: function(json) {
					...
					if (typeof profileCommon.user.presence.isPlaying !== 'undefined') {
						if (typeof profileCommon.user.presence.playingMp !== 'undefined') {
							...
							serverLink = '/bf' + (game === 2048 ? '4' : '3') +'/servers/show/pc/'+guid+'/', //if (profileCommon.user.presence.playingMp.platform === 1)
	...
	* @param  	{bool}	after	Whether it is appended or added after.
	...
	showFriends: function() {
		...
		for(var i = 0; i < length; i++) {
			...
			html += '<td>'
			html += '<a href="http://battlelog.battlefield.com/bf' + (BBLog.cache('mode') === 'bf4' ? '4' : '3') + '/user/'+player+'/">'+unescape(player)+'</a> '
			html += '</td>'
			html += '<td>'
